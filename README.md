...
## 更新日志 2026.9.14 / 1.18版本基本适配

- 修复 UTF-8 中文字符按单字节处理导致的输入/删除丢字问题
- 新增 UTF-8 安全的获取最后字符方法，避免 string.sub 截断中文字符
- 修复 Backspace、回车、选词等操作对中文字符进行错误截断的问题
- 修复已确认文本使用 Lua Pattern 匹配导致特殊字符（. [ ] % 等）匹配异常的问题
- 改用普通字符串前缀比较，避免特殊字符触发 Lua Pattern 解析
- 修复 Shift+点击任务/物品/其他聊天链接后，末尾 |r 被误识别为拼音字母 r 的问题
- 增加聊天链接检测，仅在存在 |H / |h 链接控制码且以 |r 结尾时忽略该控制码
- 保持正常输入字母 r 不受影响
